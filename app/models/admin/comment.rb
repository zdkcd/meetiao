...
class Admin::Comment < ActiveRecord::Base
  ...
  def send_sms
   if ENV['SMS_TOGGLE'] && ENV['SMS_TOGGLE'].downcase == 'true'
      ...
      if send_phone =~ /\d{11}/
        ...
        SmsSendWorker.perform_async(send_phone, "【#{Admin::Keystore.value_for('site_name') || '直达客'}】您有新预订信息,姓名:#{name},电话:#{mobile_phone},时间:#{datetime}地址:#{address},备注:#{content}")
